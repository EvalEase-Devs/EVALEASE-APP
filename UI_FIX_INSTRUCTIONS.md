...
**Status: FIXED**
**Changes made:**
- **Step 1 — Icon migration**: Replaced ALL `lucide-react` imports across 53 files with `@tabler/icons-react` equivalents. Zero `lucide-react` imports remain. Full mapping: AlertCircle→IconAlertCircle, AlertTriangle→IconAlertTriangle, ArrowLeft→IconArrowLeft, ArrowRight→IconArrowRight, AudioWaveform→IconWaveSine, BadgeCheck→IconRosetteDiscountCheck, BarChart3/BarChartBig→IconChartBar, Bell→IconBell, BookOpen→IconBook, Bot→IconRobot, Calculator→IconCalculator, Calendar→IconCalendar, Check→IconCheck, CheckCircle2→IconCircleCheck, ChevronDown→IconChevronDown, ChevronLeft→IconChevronLeft, ChevronRight→IconChevronRight, ChevronsUpDown→IconSelector, Circle→IconCircle, ClipboardCheck→IconClipboardCheck, ClipboardEdit→IconEdit, ClipboardList→IconClipboardList, Clock→IconClock, Command→IconCommand, CreditCard→IconCreditCard, DatabaseBackup→IconDatabaseExport, FileCheck→IconFileCheck, FileQuestion→IconFileSearch, FileText→IconFileText, Folder→IconFolder, Forward→IconShare, Frame→IconFrame, GalleryVerticalEnd→IconLayoutList, Hammer→IconHammer, LayoutDashboard→IconLayoutDashboard, LayoutGrid→IconLayoutGrid, Loader2→IconLoader2, LogOut→IconLogout, Map→IconMap, Microscope→IconMicroscope, Monitor→IconDeviceDesktop, MoreHorizontal→IconDots, MoreVertical→IconDotsVertical, PanelLeft→IconLayoutSidebar, Pencil→IconPencil, PieChart→IconChartPie, Play→IconPlayerPlay, Plus→IconPlus, Save→IconDeviceFloppy, ScrollText→IconNotes, Settings/Settings2→IconSettings, ShieldCheck→IconShieldCheck, Sparkles→IconSparkles, SquareTerminal→IconTerminal2, Timer→IconHourglass, Trash/Trash2→IconTrash, Upload→IconUpload, UserPlus→IconUserPlus, Users→IconUsers, X→IconX, XCircle→IconCircleX. Type mapping: LucideIcon→TablerIcon.
- **Step 2 — Icon sizes**: Applied `size` prop convention during migration (sidebar nav=18, card header=16, button w/text=16, icon-only=18, stat accent=20, empty state=40, error=48). Removed Tailwind `h-X w-X` sizing classes, kept non-sizing classes (animate-spin, text-*, mr-*, etc.) in className.
- **Step 3 — Active state fix**: In globals.css, changed `button:active, a:active, [role="button"]:active` to `button:not(.no-press):active, [role="button"]:not(.no-press):active` + added `a:active { transform: none; }` to exclude links.
- **Step 4 — .no-press utility**: Added `.no-press:active { transform: none !important; }` in `@layer utilities`. Applied `no-press` class to: SidebarTrigger Button, SidebarMenuButton base styles, BreadcrumbLink, DropdownMenuTrigger (wrapped as forwardRef component).
- **Build**: Passes with exit code 0 (42/42 pages generated).
